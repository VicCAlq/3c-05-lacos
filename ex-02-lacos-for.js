...
for (let inicio = 1; inicio <= final; inicio += 1) {
  if ( inicio % 2 != 0 ) {
  acumulador++;
}
}
...
for (let inicio = -100; inicio <= final; inicio = inicio + 1) {
   if ( inicio % 2 === 0) {
    acumulador += inicio;
   }
}
...
let mediaA = acumulador / contador;
...
for( let indice = 0; indice < itensDomesticos.length; indice += 1) {
  if(itensDomesticos[indice].startsWith(letraC)) {
    itensComC.push(itensDomesticos[indice])
  }
}
...
for(let indice = 0; indice < itensDomesticos.length; indice +=  1) {
if(indice % 2 !== 0) {
itensImpares.push(itensDomesticos[indice])
}
}
...
for( let indice = 0; indice < itensDomesticos.length; indice += 1) {
if(itensDomesticos[indice].length > 6) {
itensLongos.push(itensDomesticos[indice])
}
}
...
const itensReversa = [];
indice = itensDomesticos.length - 1


for( let indice = itensDomesticos.length - 1; indice >= 0; indice--) {
itensReversa.push(itensDomesticos[indice]);
}
...
for (let indice = 0; indice < petsExoticos.length; indice += 1) {
  petsExoticos[indice] = petsExoticos[indice].toUpperCase()
  petsMaiuscula.push(petsExoticos[indice]);
}
...
for( let indice = 0; indice < petsExoticos.length; indice += 1) {
  if(petsExoticos[indice].includes(letraR)) {
    petsComR.push(petsExoticos[indice])
  }
}
...
for (let indice = 0; indice < petsExoticos.length; indice += 1) {
  if ( petsExoticos[indice].length > 7 ) {
  pets++
}
}
